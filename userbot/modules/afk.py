...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.edit("`GW ON LAGI`\n**I'm back BISH!**")
        time.sleep(3)
        await msg.delete()
        if BOTLOG:
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"__My Master Has Been Gone.__ (Since **{afk_since}**).\nWhere He Is: ~~ONLY GOD KNOWS~~\
                        \nReason: `{AFKREASON}`")
                else:
                    await mention.reply(f"__My Master Has Been Gone.__ (Since **{afk_since}**).\nWhere He Is: ~~ONLY GOD KNOWS~~\
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"__My Master Has Been Gone.__ (Since **{afk_since}**).\nWhere He Is: ~~ONLY GOD KNOWS~~\
                        \nReason: `{AFKREASON}`")
                else:
                    await sender.reply(f"__My Master Has Been Gone.__ (Since **{afk_since}**).\nWhere He Is: ~~ONLY GOD KNOWS~~\
                        \n**Please come back later**")
